...
class Mobydick {
  ...
  require(filepath) {
    return require(path.join(this.root, filepath));
  }
    ...
    if(dirs instanceof Array) {
      for(var i in dirs) {
        if(options && options.isAbsolutePath) {
          ...
          this.statics.push(path.join(this.root, dirs[i]));
        }
      }
    } else {
      if(options && options.isAbsolutePath) {
        ...
        this.statics.push(path.join(this.root, dirs));
      }
    }
  }
  ...
  swallow(dirs, options) {
    if(dirs instanceof Array) {
      ...
      if(options && options.isAbsolutePath) {
        ...
        fullpath = path.join(this.root, dirs);
      }
