    ...
    if(dirs instanceof Array) {
      for(var i in dirs) {
        if(options && options.isAbsolutePath) {
          ...
          this.statics.push(path.join(rootDir, dirs[i]));
        }
      }
    } else {
      if(options && options.isAbsolutePath) {
        ...
        this.statics.push(path.join(rootDir, dirs));
      }
    }
  }
  ...
  swallow(dirs, options) {
    ...
    if(dirs instanceof Array) {
      ...
      if(options && options.isAbsolutePath) {
        ...
        fullpath = path.join(rootDir, dirs);
      }
...
Mobydick.prototype.digestBundle = require('./route').digest;
